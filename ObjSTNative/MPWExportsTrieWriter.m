...
@property (nonatomic, strong) NSMutableDictionary<NSString*, NSNumber*> *symbols;
@end
...
-(void)buildMultipleSymbolTrie:(NSMutableData*)trie symbols:(NSArray<NSString*>*)symbolNames
{
    ...
    // Compute where each terminal node will be
    long currentOffset = 2;  // After root header (terminalSize + numChildren)
    ...
    for (int i = 0; i < symbolNames.count; i++) {
        ...
        currentOffset += 5;  // Approximate terminal node size (termSize + flags + 2-byte addr ULEB + noChildren)
    }
...
+(int)trieSizeForSymbols:(NSArray<NSString*>*)symbols
{
    ...
    for (NSString *symbol in symbols) {
        size += 1 + (int)[symbol length] + 1 + 10;  // node info + symbol + terminator + uleb128 data
    }
